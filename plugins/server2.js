  ...
  (async () => {
    ...
    try {
      ...
      if (data.startsWith('s2_buy:')) {
        ...
        if (saldo < target.harga) {
          await bot.answerCallbackQuery(query.id, { text: `Saldo kurang. Butuh ${formatRupiah(target.harga)}`, show_alert: true });
          return;
        }
        ...
        await db.kurangSaldo(userId, target.harga);
